...
/**
 * Variante de bajo nivel usada por el login de HU-01 (`lib/api/auth.ts`) y por
 * `lib/api/transactions.ts`. Lee el mismo token que `api.*` (ver `lib/auth/token.ts`
 * y `./session.ts`), así que las dos formas de llamar a la API comparten sesión.
 */
...
export async function apiFetch<T>(path: string, options: ApiFetchOptions = {}): Promise<T> {
  const { body, onUnauthorized: onUnauthorizedForCall, headers, ...requestOptions } = options
  const token = getToken()
